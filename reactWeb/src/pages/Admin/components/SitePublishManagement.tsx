...
const SitePublishManagement: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      const remainingItems = sitePublishList.length - 1;
  ...
  const getCurrentPageData = () => {
    ...
    return sitePublishList.slice(startIndex, endIndex);
  };
  ...
  const handlePageChange = (page: number, size: number) => {
    // 如果页面大小改变，重新计算当前页
    if (size !== pageSize) {
      const newMaxPage = Math.ceil(sitePublishList.length / size);
      setCurrentPage(Math.min(page, newMaxPage));
      setPageSize(size);
    } else {
      setCurrentPage(page);
    }
      ...
      render: (value: boolean, record: SitePublish) => {
          ...
          return (
            <Tooltip title="当前默认页">
              <div className="flex items-center justify-center">
                <svg width="32" height="32" fill="#facc15" viewBox="0 0 24 24">
        ...
        return (
          <Tooltip title={record.enabled ? "点击设为默认页" : "请先启用该页面"}>
            ...
            >
              <svg width="32" height="32" fill="none" stroke="#facc15" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
              </svg>
            </div>
          </Tooltip>
